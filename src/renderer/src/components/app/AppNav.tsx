...
import { useAgents } from '@renderer/feature/agents/useAgents'
...
export default function AppNav(): JSX.Element {
  ...
  return (
    <div className="fixed inset-0 bg-black z-20 p-24 flex flex-col gap-48">
      <Button className="absolute md:left-4 right-4 top-4" onClick={() => setIsNavOpen(false)}>
        <X size={48} />
      </Button>
      <div className="space-y-2">
        ...
        <div className="text-xs text-neutral-600">
          Version commit #{commitHash.substring(commitHash.length - 6, commitHash.length)}
        </div>
      </div>
