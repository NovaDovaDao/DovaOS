        ...
        <div className="h-full px-12 md:px-8">
          {chatLog.map((message, i) => (
            ...
            >
              ...
              {chatLog.length - 1 > i && <hr className=" border-white/20 w-24 mx-auto" />}
            </article>
          ))}
          <div ref={lastEl}></div>
...
const ChatMessage = ({ message }: { message: Message }) => (
  <div
    className={`max-w-[85%] sm:max-w-[75%] py-8 ${
      message.role === 'user' ? 'text-neutral-300 ml-auto' : 'text-white mr-auto'
    }`}
  >
    <header
    ...
    <Markdown className="text-sm break-words prose prose-invert">{message.content}</Markdown>
    <footer>
      {message.role === 'agent' && (
        <span className="uppercase tracking-widest text-[10px] text-pink-400">— Dova</span>
      )}
    </footer>
  </div>
)
